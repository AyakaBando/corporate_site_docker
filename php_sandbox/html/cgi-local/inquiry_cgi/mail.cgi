...
#---------------------------------------------------------------------------------------------------
	...
	close(IN);
	$MES = "";
	$MES .= "<table class=\"dl\" border=\"0\" cellspacing=\"1\">\n";
	foreach $CK (@NEW){
		chomp $CK;
		($NAME, $VALUE) = split(/\t/,$CK);
		...
		foreach $ERR (@err){
			...
			if($ERR ne ""){
				if($ERR eq $NAME && $VALUE eq ""){
					...
					$MES .= "<tr><td>[<strong class=\"red\"> $ERR </strong>]　の項目が未入力です。</td></tr>\n";
					$mes = $MES;
				}
			}
		}
...
#---------------------------------------------------------------------------------------------------
		...
		foreach $FILELINE (@cnfile){
			if($FILELINE =~ /^(.*)<!--%(\w+)%-->(.*)$/) {
				$FILELINE = "";
				$FILELINE .= "<table class=\"mail-form\" border=\"0\" cellspacing=\"2\ cellpadding=\"5\">\n";
				$FILELINE .= $strhtml;
				$FILELINE .= "</table>\n";
				...
				$FILELINE .= "<form action=\"mail.cgi\" method=\"post\" name=\"frm\">\n" .$strvalue. "<input type=\"hidden\" name=\"check\" value=\"1\">\n</form>";
			}
			print $FILELINE;
		}
	}
...
#---------------------------------------------------------------------------------------------------
		...
		if (open(OUT,"| $g_MailCmd -t")) {
			...
			print OUT "From: $MAIL\n";
		    print OUT "Cc: $g_Mailcc\n";
		    print OUT "Bcc: $g_Mailbcc\n";
