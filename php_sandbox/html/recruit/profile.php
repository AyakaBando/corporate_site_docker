<?php require_once($_SERVER['DOCUMENT_ROOT'].'/inc/contentsConfig.php'); ?>
<!DOCTYPE html>
<html>
<head>
	<!-- Google Tag Manager -->
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','dataLayer','GTM-5LB964JX');</script>
<!-- End Google Tag Manager -->

<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1,user-scalable=no">
<meta name="format-detection" content="telephone=no">
<title>社内紹介 | 採用情報 | 森田アルミ工業</title>
<meta name="description" content="森田アルミ工業">
<meta name="Keywords" content="">
<!-- <meta property="og:title" content="社内紹介 | 採用情報 | 森田アルミ工業"> -->
<meta property="og:type" content="website">
<meta property="og:description" content="森田アルミ工業">
<meta property="og:image" content="">
<meta property="og:site_name" content="森田アルミ工業">
<link rel="shortcut icon" href="/image/icon/favicon.ico">
<link rel="apple-touch-icon" href="/image/icon/ios.png"/>
<link rel="icon" href="/image/icon/ios.png">
<link rel="stylesheet" type="text/css" href="/common/css/reset-fonts.css">
<link rel="stylesheet" type="text/css" href="/common/css/reset-min.css">
<link rel="stylesheet" type="text/css" href="/common/css/comp.css">
<link rel="stylesheet" type="text/css" href="/common/css/contents.css">
<link rel="stylesheet" type="text/css" href="/common/css/test.css">
<!--[if IE]><link rel="stylesheet" href="/common/css/fontsize_ie.css" media="all" /><![endif]-->
<!--[if lt IE 9]><script src="/common/js/html5shiv.js"></script><![endif]-->
<link href="/common/css/bootstrap.min.css" rel="stylesheet">
<link href="/common/css/bootstrap-theme.min.css" rel="stylesheet">

<link href="//netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.min.css" rel="stylesheet">
<link href='//fonts.googleapis.com/css?family=Quicksand|Source+Sans+Pro:400,200,300' rel='stylesheet' type='text/css'>
<!-- TETORI tag www.moritaalumi.co.jp -->
<script>
(function (w, d, s, u) {
  // TAG VERSION 1.00
  if (w._wsq_init_flg) {
    return false;
  }
  w._wsq_init_flg = true;
  _wsq = w._wsq || (_wsq = []);
  _wsq.push(['init', u, 2060]);
  _wsq.push(['domain', 'www.moritaalumi.co.jp']);
  var a = d.createElement(s); a.async = 1; a.charset='UTF-8'; a.src = 'https://cdn.' + u + '/share/js/tracking.js';
  var b = d.getElementsByTagName(s)[0]; b.parentNode.insertBefore(a, b);
})(window, document, 'script', 'tetori.link');
</script>
</head>
<body>
<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-5LB964JX"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<!-- End Google Tag Manager (noscript) -->


<?php require_once($_SERVER['DOCUMENT_ROOT'].'/common/include/header.php'); ?>


<section id="md_area">
	<div id="md_area_in">
		<h1>採用情報</h1>
	</div>
	<!--/md_area_in-->
</section>
<!--/md_area-->
<nav id="c_glo">
	<ul id="c_glo_in">
		<li><a href="/recruit/index.php">採用コンセプト</a></li>
		<li><a href="/recruit/flow.php">募集要項</a></li>
		<li><a href="/recruit/sales/index.php">プロジェクト紹介</a></li>
		<!-- <li><a href="/recruit/profile.php">社内紹介</a></li> -->
	</ul>
	<!--/c_glo_in-->
	<!--<br class="clearfix">-->
</nav>
<!--/c_glo-->


<article id="contents" class="recruit_flow">
<nav id="pan">
<ul>
<li><a href="/index.php">HOME</a></li>
<li><a href="/recruit/">採用情報</a></li>
<!-- <li><span>社内紹介</span></li> -->
</ul>
<br class="clearfix">
</nav><!--/pan-->

<div class="md002">
<!-- <h2 class="head">社内紹介</h2> -->
</div>
<!--/md002-->

<section class="box">
<div style=" border:solid 2px #ddd; padding:50px 20px;">
		<p class="size18 center">このページは現在制作中です。</p>
</div>
<!--<div class="ta01">
<table>
<tbody>
<tr>
<th scope="row">名称</th>
<td>森田アルミ工業株式会社</td>
</tr>
<tr>
<th scope="row">創業</th>
<td>1972年4月10日</td>
</tr>
<tr>
<th scope="row">資本金</th>
<td>3,000万円</td>
</tr>
<tr>
<th scope="row">待遇</th>
<td>傷害保険完備、１人親方労災保険の特別加入制度有り。</td>
</tr>
<tr>
<th scope="row">業者登録</th>
<td>一般建設業建具工事業 大阪府知事 許可(般-24) 第128994号</td>
</tr>
<tr>
<th scope="row">所在地</th>
<td><b>本社</b><br>
〒599-0201 大阪府阪南市尾崎町530-1[アクセスマップ]<br>
tel：072-480-1400 / fax: 072-480-1414<br>
url：<a href="http://www.moritaalumi.co.jp" target="_blank">http://www.moritaalumi.co.jp</a><br>
facebook：<a href="http://www.facebook.com/moritaalumi" target="_blank">http://www.facebook.com/moritaalumi</a><br>
twitter：<a href="http://www.twitter.com/moritaalumi" target="_blank">http://www.twitter.com/moritaalumi</a><br>
<br>
<b>ショールーム東京オフィス</b><br>
〒168-0071　東京都杉並区高井戸西1-21-13-F1<br>
tel: 03-5941-9770 / fax: 03-5941-9771</td>
</tr>
<tr>
<th scope="row">代表者</th>
<td>代表取締役社長　森田　和信</td>
</tr>
<tr>
<th scope="row">従業員数</th>
<td>45名</td>
</tr>
<tr>
<th scope="row">事業内容</th>
<td>室内・外のアルミ建材の製造販売<br>
オフィス家具の製造販売<br>
ウェディング用装飾品製造<br>
その他アルミを使用したあらゆるオーダー製品の製造</td>
</tr>
<tr>
<th scope="row">営業品目</th>
<td>
アルミ忍び返し　AG＆AGx<br>
アルミ巾木　albase<br>
グランド整備用トンボ AReco<br>
折板カーポート＆バルコニーflange port<br>
室内物干しワイヤー　pid<br>
室内物干しシェルフ　Wally<br>
デスクパーティション　patis<br>
室内パーティション　falce<br>
網戸機能付面格子　SOL<br>
アルミ室内階段　TAS<br>
エントランスフック ViK<br>
アルミ外部階段　ステアーズ<br>
バルコニー<br>
テラス<br>
ベランダ手摺<br>
手摺付笠木<br>
アルミ笠木・水切<br>
目隠しフェンス　エアリー<br>
ダイヤクロス</td>
</tr>
<tr>
<th scope="row">主要取引先</th>
<td>文化シヤッター株式会社<br>
株式会社LIXIL<br>
株式会社三協テック関西<br>
井上定株式会社<br>
杉田エース株式会社<br>
株式会社乃村工藝社<br>
株式会社イトーキ<br>
株式会社インターオフィス<br>
株式会社ワールドサービス　他</td>
</tr>
<tr>
<th scope="row">主要仕入先</th>
<td>三井物産メタルズ株式会社<br>
三和金属株式会社<br>
三菱樹脂株式会社<br>
日本ポリエステル株式会社<br>
野口金属産業株式会社<br>
タカラ産業株式会社　他</td>
</tr>
<tr>
<th scope="row">主要取引銀行</th>
<td>商工中金　堺支店<br>
三井住友銀行　佐野支店<br>
三菱東京UFJ銀行　泉佐野
支店<br>
紀陽銀行　泉南支店<br>
池田泉州銀行 阪南支店</td>
</tr>

</tbody>
</table>
</div>--><!--/ta01-->
</section><!--/box-->

<section class="box">
<!--<nav class="btn_area">
<ul>
<li><a href="" class="btn01">エントリー</a></li>
</ul>
</nav>-->
</section>



</article>
<!--/contents-->
<?php require_once($_SERVER['DOCUMENT_ROOT'].'/common/include/footer.php'); ?>
<script src="/common/js/import.js"></script>
<script src="/common/js/bootstrap.min.js"></script>
<!--<link rel="stylesheet" type="text/css" href="/common/js/jquery.bxslider.min.css">-->

<script type='text/javascript'>
piAId = '1049092';
piCId = '2117';
piHostname = 'ma.moritaalumi.co.jp';

(function() {
	function async_load(){
		var s = document.createElement('script'); s.type = 'text/javascript';
		s.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + piHostname + '/pd.js';
		var c = document.getElementsByTagName('script')[0]; c.parentNode.insertBefore(s, c);
	}
	if(window.attachEvent) { window.attachEvent('onload', async_load); }
	else { window.addEventListener('load', async_load, false); }
})();
</script>

</body>
</html>