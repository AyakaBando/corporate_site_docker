...
<?php session_start() ?>
<?php $session = $_SESSION['catalog_form'] ?>
<?php $_SESSION['catalog_form'] = array() ?>
<!DOCTYPE html>
<html>
<head>
<!-- Google Tag Manager -->
...
<article id="contents" class="privacy">
  ...
  <div class="md002">
		<h2 class="head">製品カタログ請求完了</h2>
	</div>
	<!--/md002-->

	<section class="box">
	<p class="txt02 btm40">この度はカタログ請求をいただきありがとうございました。<br>内容確認後、3営業日以内に記入いただいたご住所にご希望のカタログを送付させていただきます。<br>今しばらくお待ちくださいませ。 </p>
...
<script src="/common/js/bootstrap.min.js"></script>
  ...
  <?php if ($session) { ?>
    <iframe src="https://ma.moritaalumi.co.jp/l/1048092/2023-11-14/548i?catalog=<?php echo $session['catalog_data'] ?>&companyName=<?php echo $session['companyName'] ?>&lastname=<?php echo $session['sei'] ?>&firstname=<?php echo $session['mei'] ?>&businessCategory=<?php echo $session['businessCategory_data'] ?>&jobCategory=<?php echo $session['jobCategory_data'] ?>&busyo=<?php echo $session['busyo'] ?>&month=<?php echo $session['month'] ?>&zip=<?php echo $session['zip_data'] ?>&address=<?php echo $session['address_data'] ?>&tel=<?php echo $session['tel_data'] ?>&fax=<?php echo $session['fax_data'] ?>&url=<?php echo $session['url'] ?>&know=<?php echo $session['know_data'] ?>&comment=<?php echo str_replace("\r\n", '', $session['comment']) ?>" width="1" height="1"></iframe>
  <?php } ?>
