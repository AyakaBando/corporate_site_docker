...
if( $_POST['submitConf'] )
{
    ...
    if( array_sum( (array)$_POST['catalog'] ) >= 11 )
        $form->_errors['catalog'] = 'ご希望のカタログは10つまでしか選択できません。';
}
    ...
    if( isset( $_POST['submitReg'] ) )
    {
      ...
      $additionalText = '';
        ...
        foreach( $contactCatalogArray AS $key => $value ) {
          ...
          if($value == '建材総合カタログ') {
            // $additionalText = "<span style='color: red;'>カタログの切り替えに伴い、商品発送まで少々お待ちください。\nお急ぎの方は以下URLからダウンロードいただけます。\nhttps://ma.moritaalumi.co.jp/building_materials_general_catalog_double_side</span>";
            // $additionalText = "商品発送まで少々お待ちください。";
        }
        ...
        $adminHeader  = "From: =?UTF-8?B?" . base64_encode( ' ' ) . "?= <info-inquiry@moritaalumi.co.jp>" . "\n";
        ...
        $_SESSION['catalog_form']['businessCategory_data'] = $contactBusinessCategory[$_POST['businessCategory']];
        $_SESSION['catalog_form']['jobCategory_data'] = $contactJobCategory[$_POST['jobCategory']];
        $_SESSION['catalog_form']['address_data'] = $prefArray[$_POST['pref']] . $_POST['address1'] . $_POST['address2'];
        $_SESSION['catalog_form']['zip_data'] = $zip;
        $_SESSION['catalog_form']['tel_data'] = $tel;
        $_SESSION['catalog_form']['fax_data'] = $fax;
        $_SESSION['catalog_form']['catalog_data'] = $catalogImplode;
        $_SESSION['catalog_form']['know_data'] = $regDowmImplode;
        unset($_POST['sei']);
        unset($_POST['mei']);
