...
define( 'SQL_TABLE_NAME',  'keyWord' );
define( 'PAGE_TITLE',      'キーワード' );
define( 'FILE_CATEGORY',   'keyWord' );
...
$getLink  = createGetLink( array( 'mode', 'id', 'd', 't', 'r', 'disp', 'spotlight', 'cp', 'dispFlg' ), $_GET );
...
{
    $delet = $query->DataDelete( SQL_TABLE_NAME, $_POST, array( 'title' => 'name', 'id' => 'id' ) );
...
$dataList      = $query->KeyWordList( 0, 10000, $_GET );
...
if( $_REQUEST['id'] )
{
    $data              = $query->KeyWordDetail( $_REQUEST['id'] );
...
//エラー処理
$form->addRule( 'name',    'キーワードを入力してください。',           'required', null );
...
if ( $form->validate() )
{
    ...
    if( isset( $_POST['submitReg'] ) )
    {
        ...
        $saveParam = array(
            ...
            'data'          => $_POST, 
            'anData'        => array( 'submitConf', 'submitReg', 'submitReturn', 'reset', 'id', 'MAX_FILE_SIZE', 'imageDel', 'addItem', 'fileName', 'addContents' ), 
            'connectionKey' => array(), 
            'timeKey'       => array(), 
            'dateKey'       => array(), 
            'dateTimeKey'   => array(), 
            'fileArray'     => /*$fileArray*/array( /*'fileName', 'width', 'height'*/ ), 
        ...
        header( 'Location: ./' . THE_FILE_NAME . '.php?r=' . ( $_POST['id'] ? '2' : '1' ) . '&compId=' . $id );
        die;
    }
}
