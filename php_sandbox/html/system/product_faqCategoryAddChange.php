...
define( 'SQL_TABLE_NAME',    'product_faqcategory' );
define( 'PAGE_TITLE',        '製品FAQカテゴリ' );
define( 'FILE_CATEGORY',     'product' );
define( 'SUB_FILE_CATEGORY', '_faq' );
define( 'THE_FILE_NAME',     FILE_CATEGORY . SUB_FILE_CATEGORY . 'CategoryAddChange' );
define( 'FILE_SESSION_KEY',  FILE_CATEGORY . 'Image' );
define( 'PAGE_NUM',          4 );
define( 'SUB_PAGE_NUM',      3 );
...
if( $_GET['compId'] )
{
    $title = $query->GetCulumnContents( SQL_TABLE_NAME, 'name', 'categoryId', $_REQUEST['compId'], 1 );
}
    ...
    if( isset( $_POST['submitReg'] ) )
    {
        ...
        */
        ...
        $saveParam = array(
            ...
            'data'          => $_POST, 
            'anData'        => array( 'submitConf', 'submitReg', 'submitReturn', 'reset', 'categoryId', 'MAX_FILE_SIZE', 'imageDel', 'fileName'  ), 
            'connectionKey' => array(), 
            'timeKey'       => array(), 
            'dateKey'       => array(), 
        ...
        header( 'Location: ./' . THE_FILE_NAME . '.php?r=' . ( $_POST['categoryId'] ? '2' : '1' ) . $getLink['addition'] . '&compId=' . $compId );
        die;
    }
}
