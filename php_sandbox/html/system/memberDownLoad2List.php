...
define( 'PAGE_TITLE',      '会員ダウンロード' );
define( 'FILE_CATEGORY',   'memberDownLoad2' );
...
$memberArray          = $systemquery->GetMember( null, 1 );
$dataProduct          = $query->ProductDetailINmember( $_GET['productId'] );
...
$productArray[0] = '';
foreach( (array)$query->DataList( 0, 10000 ) AS $key => $value )
{
    $productArray[$key] = $value['name'];
}
...
$form->addElement( 'select',  'productId',         'メーカー',        $productArray, array( 'onchange' => "location.href='./" . THE_FILE_NAME . ".php?productId=' + this.value + '&memberId=" . $_GET['memberId'] . "'" ) );
